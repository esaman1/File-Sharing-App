...
public class MaxHeightLayout extends FrameLayout {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                com.datasharing.R.styleable.MaxHeightLayout);

        final int count = a.getIndexCount();
        for (int i = 0; i < count; ++i) {
            int attr = a.getIndex(i);
            if (attr == com.datasharing.R.styleable.MaxHeightLayout_mhl_HeightRatio) {
                mMaxRatio = a.getFloat(attr, DEFAULT_MAX_RATIO);
            } else if (attr == com.datasharing.R.styleable.MaxHeightLayout_mhl_HeightDimen) {
